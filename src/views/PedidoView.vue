...
// Interface completa do payload do pedido
...
onMounted(() => {
  const rawPayload = (route.query.payload as string) || ''
  if (rawPayload) {
    try {
      const decoded = decodeURIComponent(rawPayload)
      const parsed = JSON.parse(decoded) as OrderPayload
      order.value = parsed
    } catch (e) {
...
// Função auxiliar para formatar texto da forma de pagamento
...
<style scoped>
/* Você pode ajustar espaçamentos ou estilos conforme necessário */
</style>
